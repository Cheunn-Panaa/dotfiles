...
function __kustomize_perform_completion
    __kustomize_debug "Starting __kustomize_perform_completion"
...
# - Obtain the completions and store them in the global __kustomize_comp_results
...
function __kustomize_prepare_completions
    ...
    set -l results (__kustomize_perform_completion)
    ...
    if test $filefilter -eq 1; or test $dirfilter -eq 1
        __kustomize_debug "File extension filtering or directory filtering not supported"
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __kustomize_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__kustomize_comp_results)
        set --global __kustomize_comp_results $completions
        __kustomize_debug "Filtered completions are: $__kustomize_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__kustomize_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__kustomize_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __kustomize_comp_results $split[1] $split[1].
                __kustomize_debug "Completions are now: $__kustomize_comp_results"
            end
        end
...
if type -q "kustomize"
    ...
    complete --do-complete "kustomize " > /dev/null 2>&1
end

# Remove any pre-existing completions for the program since we will be handling all of them.
complete -c kustomize -e

# The call to __kustomize_prepare_completions will setup __kustomize_comp_results
